...
const readVideoIdFromWatchEndpoint = (inputNode) => {
    // 1) Try decoding a node with direct `watchEndpoint.videoId`.
    // 2) Return that id when present.
    // 3) Return null when structure doesn't match.
    try {
...
const readVideoIdFromWatchCommand = (inputNode) => {
    // 1) Try decoding a node with `videoCommand.watchEndpoint.videoId`.
    // 2) Return that id when present.
    // 3) Return null when structure doesn't match.
    try {
...
const parsePlaylistRenderer = (inputNode) => {
    // 1) Validate node against the expected renderer schema.
    // 2) Return videoId when valid.
    // 3) Return null on mismatch so traversal can continue.
    try {
...
const asObjectRecord = (inputNode) => {
    // 1) Try to treat node as a generic object record.
    // 2) Return typed object when decode succeeds.
    // 3) Return null to skip non-object branches in recursion.
    try {
...
export const collectPlaylistVideoIds = (playlistPayload, idsCollector) => {
    // 1) Keep a Set of seen IDs while mutating the output array.
    // 2) Recursively visit arrays and objects, extracting playlistRenderer IDs.
    // 3) Push each new ID once to avoid duplicates.
